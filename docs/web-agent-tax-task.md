...
## 追加步骤: 当季预计算(三层口径第②层, 用户拍板 2026-07-15)

报送页当季通常还没出数。对**当季**(如 Q3)追加抓「收支账单·收入」页(第一个网址):
`https://myseller.taobao.com/home.htm/whale-accountant/bill/summary?billDirection=income&billType=month`

1. 逐月选当季已过/进行中的月份(如 7月、8月…),读每月「收入 − 退款」;
2. 各月净额求和 = 当季预计算净额;
3. 输出到 quarters 里, **务必带 `"provisional": true`**(ERP 据此标"预计算", 报送出数后自动覆盖):

```json
"2026-Q3": {"net_income": 45530.00, "provisional": true}
```

> 三层口径全景: 过去季=报送定数 / 当季=本页预计算(打款口径) / 在途订单 2% 由 ERP
> 减项「待扣2%补贴税(在途)」实时计提(不经本任务)。
