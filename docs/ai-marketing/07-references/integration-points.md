...
## ⭐ 最高优先级对接：Panse-System ERP（家具品牌核心系统）

> 评审补充（2026-06-10）：原文档遗漏了与品牌主业 ERP 的对接，而这是把营销闭环从"内容"延伸到"生意"的关键，也是本品牌相对通用内容工厂的独有优势。

**关系**：强依赖 / 双向。营销系统的成交归因与 ROI 必须接 ERP，否则闭环止于平台互动数据。

**对接点**：
- 项目：`Panse-System`（家具电商内部 ERP，FastAPI + PostgreSQL）
- **产品数据入向**：营销系统的产品线关键词库 / 选题素材 / 视觉锚定的产品图，源头是 ERP 的产品表（`products`）、定价表（`pricing_sku`）、SKU 图——不另维护第二份产品资料。
- **成交归因出向**：⑩ 线索收件箱的成交线索回写 ERP 订单号（line ↔ `orders.order_no`）。
- **ROI 复用**：营销内容投入（薯条费用 / 人工时）与归因订单 → 复用 ERP 现成的 `roi_service.compute` / `roi_service.monthly_breakdown`，形成"内容 → 订单 → ROI"统一报表，**不在营销系统重造 ROI**。
- **待办清单模式复用**：⑨ 养号 SOP 引擎、⑧ 评论引流站的人工任务，直接复用 ERP 的 `ops_checklist_service.py` 模式（`system_settings` 存 JSON、跨周期自动重置、免建表迁移）。

**对接时间**：Phase 1（从一开始就接，哪怕先手工登记一条"内容→订单"样本）。

**注意**：营销系统不建自己的订单/财务表，订单与对账真相永远在 ERP，避免双算口径打架。
