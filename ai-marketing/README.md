...
左侧分组菜单 + 「🏠今日待办」首页（一屏看今天该做什么，点卡片直达）。每个页面顶部都有**黄色操作说明**，照做即可：

- **内容生产**：🔥爆文挖掘（竞品爆款拆解+低粉爆文+评论词云）→ ①②选题与生成（图文/口播脚本）→ ③审核工位（标题A/B）→ ④发布队列（错峰+卡片+数据自动回采）→ 📆内容日历（账号×日期排期）
- **引流获客**：⑤评论引流 → 💌评论管理（自有笔记评论：意图分类+话术回复+转线索+楼中楼）→ 📡品牌舆情（品牌/竞品提及监控）→ ⑥线索收件箱 → ⑦知乎占坑
- **账号运营**：⑧养号清单（矩阵互动换量+差异化节奏）→ ⑨账号管理（真人/设备/官方功能+一机多号告警+风控熔断）→ ⑩运营台账
- **辅助工具**：📅大促日历 / 🛡文案合规自查 / 📊数据与复盘 / ⚙️系统状态

> Phase2 对标高星项目（[MediaCrawler](https://github.com/NanmiCoder/MediaCrawler) 采集 / [XiaoFeiShu](https://github.com/Jici-Zeroten/XiaoFeiShu) 养号运营 / [Postiz](https://github.com/gitroomhq/postiz-app)·[Mixpost](https://github.com/inovector/mixpost) 分发）落地 20 项内容影响功能，覆盖养号/内容制作/分发/评论四块。**采集只读、发布与评论永远人工点发。**

数据源：默认演示数据；配 `CRAWLER_BASE_URL` 接入「AI 数据爬虫」后自动切真实上升期笔记（只读不发）。
通知：配 `FEISHU_WEBHOOK_URL` 后看门狗告警/超期线索/到点发布推送到飞书群。
