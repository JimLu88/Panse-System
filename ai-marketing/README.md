...
- **内容生产**：①②选题与生成 → ③审核工位 → ④发布队列（自动错峰+复制卡片人工发）
- **引流获客**：⑤评论引流（找机会+AI写稿+人工发）→ ⑥线索收件箱（话术库一键复制+成交回填订单）→ ⑦知乎占坑（20个高搜索问题）
- **账号运营**：⑧养号清单 → ⑨账号管理（真人/设备/官方功能登记）→ ⑩运营台账（日/周/月打卡：实拍素材日/复盘会/投放记账/设备盘点）
- **辅助工具**：📅大促日历（提前45天种草提醒）/ 🛡文案合规自查（广告法红线）/ 📊数据与复盘 / ⚙️系统状态（看门狗+数据源）
