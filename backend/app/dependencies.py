...
CAMPAIGN_PLAN7_FINAL_CLOSEOUT_PATH = (
    "/api/campaigns/execute-super-reduce-plan7-final-closeout"
)
CAMPAIGN_PLAN7_FINAL_CLOSEOUT_V3_PATH = (
    "/api/campaigns/execute-super-reduce-plan7-final-closeout-v3"
)
...
CAMPAIGN_PLAN8_FINAL_RECOVERY_V6_PATH = (
    "/api/campaigns/recover-super88-plan8-final-v6"
)
CAMPAIGN_PLAN8_FINAL_RECOVERY_V6_CLAIM_VERIFY_PATH = (
    "/api/campaigns/recover-super88-plan8-final-v6/claim-verification"
)
CAMPAIGN_PLAN8_FINAL_RECOVERY_V7_PATH = (
    "/api/campaigns/recover-super88-plan8-final-v7"
)
CAMPAIGN_PLAN8_FINAL_RECOVERY_V7_CLAIM_VERIFY_PATH = (
    "/api/campaigns/recover-super88-plan8-final-v7/claim-verification"
)
CAMPAIGN_PLAN8_FINAL_RECOVERY_V8_PATH = (
...
CAMPAIGN_PLAN7_DISCOUNT_TIME_RECOVERY_PATH = (
    "/api/campaigns/recover-super-reduce-plan7-discount-times"
)
CAMPAIGN_PLAN7_DISCOUNT_TIME_RECOVERY_V2_PATH = (
    "/api/campaigns/recover-super-reduce-plan7-discount-times-v2"
)
...
CAMPAIGN_WAREHOUSE_PRODUCT_PRICE_CORRECTION_PATH = (
    "/api/campaigns/correct-warehouse-product-sku-price"
)
CAMPAIGN_FIVE_PRICE_CORRECTION_PATH = (
    "/api/campaigns/correct-five-price-issues"
)
CAMPAIGN_FIVE_PRICE_RECOVERY_PATH = (
...
def machine_identity_for_key(
    ...
    *,
    ...
    if path == CAMPAIGN_PLAN8_FINAL_RECOVERY_V6_CLAIM_VERIFY_PATH:
        expected = settings_service.get(db, "web_agent_token", env_fallback=True)
        if expected and hmac.compare_digest(candidate, expected.strip()):
            return "machine:web-agent-plan8-v6-claim-verify"
    if path == CAMPAIGN_PLAN8_FINAL_RECOVERY_V7_CLAIM_VERIFY_PATH:
        expected = settings_service.get(db, "web_agent_token", env_fallback=True)
        if expected and hmac.compare_digest(candidate, expected.strip()):
            return "machine:web-agent-plan8-v7-claim-verify"
    if path == CAMPAIGN_PLAN8_FINAL_RECOVERY_V8_CLAIM_VERIFY_PATH:
        expected = settings_service.get(db, "web_agent_token", env_fallback=True)
        if expected and hmac.compare_digest(candidate, expected.strip()):
...
def require_web_agent_plan8_v6_claim_verifier(
    request: Request,
    x_api_key: Optional[str] = Header(None, alias="X-API-Key"),
    db: Session = Depends(get_db),
) -> ServicePrincipal:
    identity = machine_identity_for_key(x_api_key, db, path=request.url.path)
    if identity != "machine:web-agent-plan8-v6-claim-verify":
        raise HTTPException(status.HTTP_401_UNAUTHORIZED,
                            "Web-Agent计划8 V6核验身份无效")
    return ServicePrincipal(
        username=identity, role="web_agent_service",
        scope="campaign.super88.plan8.final_recovery_v6.claim_verify.readonly",
    )


def require_web_agent_plan8_v7_claim_verifier(
    request: Request,
    x_api_key: Optional[str] = Header(None, alias="X-API-Key"),
    db: Session = Depends(get_db),
) -> ServicePrincipal:
    identity = machine_identity_for_key(x_api_key, db, path=request.url.path)
    if identity != "machine:web-agent-plan8-v7-claim-verify":
        raise HTTPException(status.HTTP_401_UNAUTHORIZED,
                            "Web-Agent计划8 V7核验身份无效")
    return ServicePrincipal(
        username=identity, role="web_agent_service",
        scope="campaign.super88.plan8.final_recovery_v7.claim_verify.readonly",
    )


def require_web_agent_plan8_v8_claim_verifier(
    request: Request,
    x_api_key: Optional[str] = Header(None, alias="X-API-Key"),
    db: Session = Depends(get_db),
) -> ServicePrincipal:
    identity = machine_identity_for_key(x_api_key, db, path=request.url.path)
    if identity != "machine:web-agent-plan8-v8-claim-verify":
        raise HTTPException(status.HTTP_401_UNAUTHORIZED,
                            "Web-Agent计划8 V8核验身份无效")
    return ServicePrincipal(
        username=identity, role="web_agent_service",
...
def require_campaign_prepare_principal(
    ...
    if identity == "service:campaign-prepare":
        path_scopes = {
            ...
            CAMPAIGN_PLAN7_FINAL_CLOSEOUT_PATH: (
                "campaign.super_reduce.plan7.final_closeout"),
            CAMPAIGN_PLAN7_FINAL_CLOSEOUT_V3_PATH: (
            ...
            CAMPAIGN_FIVE_PRICE_CORRECTION_PATH: (
                "campaign.five_price_issues.correct"),
            CAMPAIGN_FIVE_PRICE_RECOVERY_PATH: (
                "campaign.five_price_issues.recover_single_discount"),
            CAMPAIGN_FIVE_PRICE_SUPER_RECOVERY_PATH: (
                "campaign.five_price_issues.recover_super_reduce"),
            CAMPAIGN_FIVE_PRICE_SUPER_RECOVERY_V2_PATH: (
