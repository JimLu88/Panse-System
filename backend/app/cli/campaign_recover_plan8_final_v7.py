"""Run the fixed Plan 8 V7 recovery through the local API."""
...
from app.services import campaign_plan8_final_recovery_v7_service as recovery
from app.services import settings_service


_URL = "http://127.0.0.1:8000/api/campaigns/recover-super88-plan8-final-v7"
...
def _read_payload() -> bytes:
    ...
    if not raw or len(raw) > _MAX_INPUT_BYTES:
        raise ValueError("计划8最终恢复V7 JSON缺失或超过4 KiB")
    try:
        body = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"计划8最终恢复V7 JSON无效: {exc}") from exc
    expected = {
    ...
    if body != expected or body.get("mode") not in {"execute", "readback"}:
        raise ValueError("计划8最终恢复V7输入与程序固化范围不一致")
    ...
    try:
        ...
        req = request.Request(
            ...
            headers={"Content-Type": "application/json; charset=utf-8",
                     ...
                     "User-Agent": "panse-campaign-plan8-final-recovery-v7/1"})
