"""Call the fixed one-shot plan-7 small-promo correction API."""
...
_URL = "http://127.0.0.1:8000/api/campaigns/correct-super-reduce-plan7-small-promo"
...
def call_api(*, token: str) -> tuple[int, bytes]:
    payload = json.dumps(service.request_payload(), ensure_ascii=False,
                         separators=(",", ":")).encode("utf-8")
    req = request.Request(_URL, data=payload, method="POST", headers={
        "Content-Type": "application/json; charset=utf-8",
        "X-API-Key": token,
        "User-Agent": "panse-plan7-small-promo-correction-cli/1",
    })
    try:
