"""Call the incident-scoped plan-7 V4 closeout endpoint exactly once."""
...
_URL = "http://127.0.0.1:8000/api/campaigns/execute-super-reduce-plan7-final-closeout-v4"
_MAX_INPUT_BYTES = 12 * 1024
_FIXED_PAYLOAD = service.request_payload()


def _read_payload() -> bytes:
    raw = sys.stdin.buffer.read(_MAX_INPUT_BYTES + 1)
    if not raw:
        raise ValueError("stdin 必须提供计划7最终收口 V4 JSON")
    if len(raw) > _MAX_INPUT_BYTES:
        raise ValueError("计划7最终收口 V4 JSON 超过 12 KiB 上限")
    try:
        body = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"计划7最终收口 V4 JSON 无效: {exc}") from exc
    if body != _FIXED_PAYLOAD:
        raise ValueError("计划7最终收口 V4 输入与固化恢复身份不一致")
    return json.dumps(
...
def call_closeout(payload: bytes, *, token: str) -> tuple[int, bytes]:
    req = request.Request(
        ...
        headers={
            ...
            "X-API-Key": token,
            "User-Agent": "panse-campaign-plan7-final-closeout-cli/4",
        })
