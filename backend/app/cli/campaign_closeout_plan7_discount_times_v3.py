"""Call the exact read-only V3 closeout for the submitted plan-7 attempt."""
...
_URL = (
    ...
    "closeout-super-reduce-plan7-discount-times-v3"
)
...
def _read_payload() -> bytes:
    ...
    if not raw or len(raw) > _MAX_INPUT_BYTES:
        raise ValueError("stdin 必须提供计划7三活动V3只读收口 JSON")
    try:
        body = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"计划7三活动V3只读收口 JSON 无效: {exc}") from exc
    normalized = (
        campaign_plan7_time_update_service.normalize_recovery_v3_request(body)
    )
    return json.dumps(
...
def call_api(payload: bytes, *, token: str) -> tuple[int, bytes]:
    req = request.Request(
        ...
        headers={
            ...
            "X-API-Key": token,
            "User-Agent": "panse-plan7-discount-time-readback-v3-cli/1",
        })
