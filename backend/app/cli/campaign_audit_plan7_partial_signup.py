"""Read the official failure workbook and enrolled state for one plan-7 attempt."""
...
_URL = (
    ...
    "audit-super-reduce-plan7-partial-signup"
)
...
_FIXED_PAYLOAD = {
    ...
    "expected_manifest_sha256": service.PARTIAL_MANIFEST_SHA256,
}
...
def _read_payload() -> bytes:
    ...
    if not raw or len(raw) > _MAX_INPUT_BYTES:
        raise ValueError("stdin 必须提供不超过8KiB的计划7部分导入核对JSON")
    try:
        body = json.loads(raw)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise ValueError(f"计划7部分导入核对JSON无效: {exc}") from exc
    if body != _FIXED_PAYLOAD:
        raise ValueError("输入与程序固化的计划7部分导入核对范围不一致")
...
def call_audit(payload: bytes, *, token: str) -> tuple[int, bytes]:
    req = request.Request(
        _URL, data=payload, method="POST",
        headers={
            "Content-Type": "application/json; charset=utf-8",
            "X-API-Key": token,
            "User-Agent": "panse-campaign-plan7-partial-audit-cli/1",
        },
    )
...
def main() -> int:
    try:
        ...
        status, body = call_audit(payload, token=_service_token())
