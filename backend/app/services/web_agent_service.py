...
def product_sku_slot_draft_recover(
        db: Session, payload: dict, *, timeout_s: int = 600) -> dict:
    """Repair the one exact existing platform draft without publishing it."""
    job = _post(db, "/api/product-sku/slot-draft-recover", dict(payload), timeout=30)
    if not job.get("ok") or not job.get("job"):
        return {"ok": False, "error": job.get("error", "Web-Agent未响应"),
                "platform_product_write": False}
    final = wait_job(db, job["job"], timeout_s=timeout_s)
    ...
    if final.get("status") in ("error", "failed") and not result:
        return {"ok": False,
                "error": final.get("error") or "sku_slot_draft_recovery_failed",
                "job_id": job["job"], "platform_product_write": False}
    return {**result, "job_id": job["job"]}
...
def supplement_plan7_single_discount(
        db: Session, *, payload: dict, timeout_s: int = 1800) -> dict:
    """Run the fixed plan-7 existing-activity add-product job once."""
    started = _post(
        db, "/api/single-item-discount/plan7-supplement",
        payload, timeout=30)
    job_id = started.get("job")
    if not started.get("ok") or not job_id:
        return {
...
def supplement_plan7_sample_cent_single_discount(
        db: Session, *, payload: dict, timeout_s: int = 1800) -> dict:
    """Run the fixed four-SKU 5.99 sample repair once."""
    started = _post(
        db, "/api/single-item-discount/plan7-sample-cent-supplement",
        payload, timeout=30)
    job_id = started.get("job")
    if not started.get("ok") or not job_id:
        return {
...
def recover_plan8_final_v3(
    ...
    """Run one phase of the fixed plan-8 V3 draft recovery.

    ``inspect`` and ``readback`` are read-only. ``commit`` is the only phase
...
def recover_plan8_final_v6(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """Run one phase of the independent plan-8 V6 recovery."""
    started = _post(
        db, "/api/campaign/plan8-final-recovery-v6", payload, timeout=30)
    if not started.get("ok") or not started.get("job"):
        busy = (
            started.get("error") == "taobao_profile_busy"
            and started.get("step") in {"pre_write_busy", "readback_busy"}
        )
        return {
            **started,
            "ok": False,
            "busy": busy,
            "pre_write_busy": busy and payload.get("phase") == "inspect",
            "platform_write": False if busy else started.get("platform_write"),
        }
    job_id = str(started["job"])
    final = wait_job(db, job_id, timeout_s=timeout_s)
...
def recover_plan8_final_v7(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """Run one phase of the independent plan-8 V7 recovery."""
    started = _post(
        db, "/api/campaign/plan8-final-recovery-v7", payload, timeout=30)
    if not started.get("ok") or not started.get("job"):
        busy = (
            started.get("error") == "taobao_profile_busy"
            and started.get("step") in {"pre_write_busy", "readback_busy"}
...
def recover_plan8_final_v8_preupload_resume_v10(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V10-only claimed-preupload continuation after strict guard settling."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v10")


def recover_plan8_final_v8_preupload_resume_v12(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V12-only continuation after the lazy file-input stop."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v12")


def recover_plan8_final_v8_preupload_resume_v14(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V14-only continuation after exact semantic modal binding repair."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v14")


def recover_plan8_final_v8_preupload_resume_v15(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V15-only continuation after the read-only editor label-format stop."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
...
def recover_plan8_final_v8_preupload_resume_v17(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V17-only continuation after the 下载模版 text-variant stop."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
...
def recover_plan8_final_v8_preupload_resume_v21(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V21 continuation after the V20 ERP claim-step bookkeeping fix."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
...
def recover_plan8_final_v8_preupload_resume_v24(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V24 continuation after V23's read-only platform export failed."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v24")


def recover_plan8_final_v8_preupload_resume_v25(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V25 continuation using the exact reviewed manual pre-write export."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v25")


def recover_plan8_final_v8_preupload_resume_v26(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V26 continuation after ERP fixed manual-export field forwarding."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v26")


def recover_plan8_final_v8_preupload_resume_v27(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V27 continuation with bounded exact-editor hydration waiting."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v27")


def recover_plan8_final_v8_preupload_resume_v28(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """V28 continuation after the ERP V27 claim-step allowlist repair."""
    return _recover_plan8_final_v8_preupload_resume(
        db, payload=payload, timeout_s=timeout_s,
        path="/api/campaign/plan8-final-recovery-v8-preupload-resume-v28")
...
def correct_five_price(
        db: Session, *, payload: dict, timeout_s: int = 2400) -> dict:
    """Run one exact phase of the reviewed five-item correction."""
    started = _post(
        db, "/api/campaign/five-price-correction", payload, timeout=30)
    job_id = started.get("job")
    if not started.get("ok") or not job_id:
        return {"ok": False,
                "error": started.get("error", "取数服务(:8500)未响应"),
