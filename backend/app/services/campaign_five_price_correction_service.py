...
SUPER_RECOVERY_V3_EXPECTED_TARGETS = {
    "1046992283533": {"6241476755540": "285.00"},
    "717418169535": {"5011017605466": "400.00"},
    "840643621692": {
        "5606206268612": "277.00",
        "5917906151868": "277.00",
...
def super_recovery_v3_request_payload() -> dict:
    return {
        **request_payload("super_reduce"),
        "expected_failed_attempt_id": SUPER_RECOVERY_V3_FAILED_ATTEMPT_ID,
        "confirmed_partial_persisted_change": True,
        "persisted_item_id": "1046992283533",
    }


def validate_super_recovery_v3_request(payload: dict) -> bool:
    return (
        isinstance(payload, dict)
...
def _super_v2_partial_persist_attempt_exact(
        attempt: CampaignExecutionAttempt) -> bool:
    summary = attempt.result_summary if isinstance(
        attempt.result_summary, dict) else {}
    terminal_error = str(summary.get("terminal_error") or "")
    return bool(
        attempt.id == SUPER_RECOVERY_V3_FAILED_ATTEMPT_ID
        and attempt.plan_id == PLAN_ID
        and attempt.workflow_key == WORKFLOW_KEY
        and attempt.operation == SUPER_RECOVERY_V2_OPERATION
        and attempt.scope_sha256 == hashlib.sha256(json.dumps(
            super_recovery_v2_request_payload(), sort_keys=True,
            separators=(",", ":")).encode()).hexdigest()
        and attempt.state == "unknown"
        and attempt.write_claimed is True
        and attempt.platform_write_observed is True
        and attempt.web_agent_job_id == "job6"
        and attempt.automatic_retry_allowed is False
        and attempt.last_step == "super_recovery_v2_terminal_not_exact"
        and str(attempt.error_code or "").startswith(
            "TimeoutError: Locator.click: Timeout 5000ms exceeded.")
        and summary.get("request") == super_recovery_v2_request_payload()
        and summary.get("failed_attempt_id") == (
            SUPER_RECOVERY_V2_FAILED_ATTEMPT_ID)
...
def _readback_super_v3_expected_targets(db: Session) -> dict:
    evidence = []
    for item_id, targets in SUPER_RECOVERY_V3_EXPECTED_TARGETS.items():
        result = web_agent_service.inspect_super_reduce_item(
            db, item_id=item_id, timeout_s=300)
        if not result.get("ok"):
            return _fail(
                "five_price_super_recovery_v3_readback_failed",
                phase="super_reduce", item_id=item_id,
                readback_error=result.get("error"),
                web_agent_job_id=result.get("web_agent_job_id"))
        if result.get("status") != "活动中":
            return _fail(
                "five_price_super_recovery_v3_item_not_active",
                phase="super_reduce", item_id=item_id,
                actual_status=result.get("status"))
        inventory = result.get("editor_inventory") or {}
        rows = inventory.get("rows") or []
        ...
        if duplicate_skus:
            return _fail(
                "five_price_super_recovery_v3_duplicate_sku",
                phase="super_reduce", item_id=item_id,
                duplicate_sku_ids=sorted(duplicate_skus))
        mismatch = {
            sku_id: {"expected": expected, "actual": by_sku.get(sku_id)}
...
def recover_super_reduce_v3(db: Session, *, payload: dict) -> dict:
    """Recover only the three items left old after one proven saved item."""
    if not validate_super_recovery_v3_request(payload):
        return _fail("five_price_super_recovery_v3_request_not_allowed",
                     phase="super_reduce")
    plan_error = _validate_plan(db)
    if plan_error:
        return plan_error
    if not _single_discount_completed(db):
        return _fail("five_price_single_discount_not_completed",
                     phase="super_reduce")
    failed = db.execute(select(CampaignExecutionAttempt).where(
        CampaignExecutionAttempt.id == SUPER_RECOVERY_V3_FAILED_ATTEMPT_ID,
    ).with_for_update()).scalar_one_or_none()
    if failed is None:
        return _fail("five_price_super_recovery_v3_failed_attempt_not_found",
                     phase="super_reduce")
    if not _super_v2_partial_persist_attempt_exact(failed):
        return _fail(
    ...
    existing = db.execute(select(CampaignExecutionAttempt).where(
        CampaignExecutionAttempt.workflow_key == WORKFLOW_KEY,
        CampaignExecutionAttempt.operation == SUPER_RECOVERY_V3_OPERATION,
        CampaignExecutionAttempt.scope_sha256 == scope_sha,
    ).with_for_update()).scalar_one_or_none()
    if existing:
        return _fail("five_price_super_recovery_v3_already_consumed_no_retry",
                     phase="super_reduce", attempt_id=existing.id,
                     attempt_state=existing.state,
                     platform_write=existing.platform_write_observed)

    # The first item must be the one exact persisted change, while all seven
    # remaining target SKU values must still be their fixed originals.
    ...
    attempt = CampaignExecutionAttempt(
        id=attempt_id, plan_id=PLAN_ID, workflow_key=WORKFLOW_KEY,
        operation=SUPER_RECOVERY_V3_OPERATION, scope_sha256=scope_sha,
        state="write_claimed", write_claimed=True,
        write_claimed_at=datetime.now(timezone.utc),
        platform_write_observed=False, automatic_retry_allowed=False,
        request_id=f"five-price-recover-super-v3-{secrets.token_hex(6)}",
        last_step="one_new_three_old_readback_claimed",
        result_summary={
            "request": payload,
            "failed_attempt_id": SUPER_RECOVERY_V3_FAILED_ATTEMPT_ID,
            "failed_attempt_preserved": True,
            "preflight_readback": preflight,
            "persisted_item_id": "1046992283533",
            "remaining_item_ids": [
    ...
    try:
        ...
        db.rollback()
        return _fail("five_price_super_recovery_v3_claim_raced_no_write",
                     phase="super_reduce")
    ...
    attempt.last_step = (
        "exact_editor_readback" if exact
        else "super_recovery_v3_terminal_not_exact")
    attempt.error_code = None if exact else str(
        terminal.get("error")
        or "five_price_super_recovery_v3_terminal_not_exact")[:128]
    attempt.result_summary = {
        **(attempt.result_summary or {}),
        "terminal_ok": terminal.get("ok"),
        "submitted": terminal.get("submitted"),
    ...
    if not exact:
        return _fail(
            "five_price_super_recovery_v3_terminal_not_exact_no_retry",
            phase="super_reduce", attempt_id=attempt_id,
            platform_write=observed, terminal=terminal)
    return {
        "ok": True, "phase": "super_reduce",
