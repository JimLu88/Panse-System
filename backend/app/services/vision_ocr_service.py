...
_BALANCE_SYSTEM = (
    ...
    "  · 含『推广/万相台』→ 页面可能是旧版『万相台无界版·账户总余额』，也可能是新版『推广业务账户』里的"
    "『万相台(元)』。只取明确紧挨『万相台』标签的那个数；绝不能取同一区域的『直通车』『超级推荐』，"
    "也不能取页面上方『聚合结算账户』的余额。明确找到万相台标签和数字才是 high;\n"
    "  · 含『保证金』→ 保证金账户「可用余额」;\n"
...
def parse_balance_screenshot(
    ...
    if "推广" in _hint or "万相台" in _hint:
        ...
        if ("万相台" not in _label
                or any(k in _label for k in (
                    "聚合", "保证金", "可提现", "冻结", "直通车", "超级推荐",
                ))):
            data["confidence"] = "low"
            data["note"] = (str(data.get("note") or "") +
                            f" | 反向校验: 推广余额只认明确的万相台余额, 却读到「{_label}」→ 判低置信不入库")
    return data
