...
OPS_TASKS = [
    # —— 每日 ——
    {"key": "daily_import_orders", "freq": "daily", "title": "导入当日淘宝订单",
     "detail": "订单→CSV导入: 上传当天淘宝订单表(销售明细/订单CSV), 自动补金额+客户"},
    {"key": "daily_screenshot", "freq": "daily", "title": "截图录单",
     "detail": "把零散/手工订单截图上传(截图录单), 录入系统"},
    {"key": "daily_logistics", "freq": "daily", "title": "处理发货 / 查物流",
     "detail": "订单看板看待发货 → 发货并查快递; 处理在途异常"},
    {"key": "daily_aftersales", "freq": "daily", "title": "处理售后 / 退货",
     "detail": "退货/售后页: 新建退货、确认签收入库、登记 客户退回/我方补发 快递"},
    # —— 每周 ——
    {"key": "weekly_part_stock", "freq": "weekly", "title": "配件库存盘点 / 补货",
     "detail": "配件库存→智能预警: 看缺货/低于预警线, 下采购补货"},
    {"key": "weekly_recon", "freq": "weekly", "title": "对账(订单 ↔ 收款)",
     "detail": "逐笔核对订单实付 ↔ 支付宝企业号(9A)/微信账单 到账, 标差额"},
    {"key": "weekly_price", "freq": "weekly", "title": "竞品调价",
     "detail": "定价表: 按竞品多选批量改大促价 / 套用公式"},
    # —— 每月 ——
    {"key": "monthly_alipay", "freq": "monthly", "title": "导入支付宝流水",
     "detail": "导入 企业号(9A) / 爱群号(9C) 当月支付宝流水"},
    {"key": "monthly_wechat_bill", "freq": "monthly", "title": "导入微信账单(billDetail)",
     "detail": "导入当月微信支付订单账单明细(billDetail xlsx)"},
    {"key": "monthly_recon", "freq": "monthly", "title": "月度对账 + 推广ROI",
     "detail": "全月逐笔对账; 算推广支出占正式销售额(不含补单)的占比"},
    {"key": "monthly_investment", "freq": "monthly", "title": "更新总投资费用",
     "detail": "财务→剩余流水: 编辑总投资费用/保证金, 重新测算可用资金"},
    {"key": "monthly_product_stock", "freq": "monthly", "title": "成品库存月度盘点",
