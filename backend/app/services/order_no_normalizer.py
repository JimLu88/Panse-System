...
def is_non_order_reference(
    ...
    判据 (任一命中即 True):
      ...
      4) 支付渠道内部号形态: ≥20 位纯数字(淘宝单号恒 18-19 位)。
    18/19 位纯数字不在此判 —— 交由调用方结合账户判断, 避免误伤真单。
    """
    ...
    if rel.startswith("P") or re.search(r"[A-Za-z=+/]", rel):
        return True
    # 4) 支付渠道内部号 (2026-07-10): ≥20 位纯数字天生不是淘宝订单号(恒 18-19 位) —— 支付宝账务
    # 流水号(2026042120004001…32位)/交易号(2026042122001488…28位)/微信支付号(4200…28位)/微信转账
    # 通道号(1000050001…31位)/收钱码(4064…22位)/理财号(4767…23位)全属此类。爱群号/佳宝号个人账单的
    # 「商户订单号」列装的就是这些, 655 条被僵尸提示"待补还原规则"——其实无规则可补。到达此处前
    # resolve_platform_order_no 已尝试过所有抽取规则(含长串内嵌 19 位核心)并失败, 故此判不丢真单。
    if re.fullmatch(r"\d{20,}", rel):
        return True
    return False
