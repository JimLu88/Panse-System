...
from app.cli import campaign_recover_plan8_final_v5 as cli
from app.models.campaign import CampaignExecutionAttempt, CampaignPlan
from app.services import (
    campaign_execution_service,
    campaign_plan8_final_recovery_v5_service as recovery,
...
def test_plan8_v5_route_is_narrowly_allowlisted_and_v4_is_preserved():
    assert dependencies.CAMPAIGN_PLAN8_FINAL_RECOVERY_V5_PATH == (
    ...
    assert dependencies.CAMPAIGN_PLAN8_FINAL_RECOVERY_V4_PATH in (
        dependencies.CAMPAIGN_PREPARE_SERVICE_PATHS)
    assert recovery.RECOVERY_VERSION == 5
    assert recovery.OPERATION == "plan8_final_recovery_v5"
...
def test_plan8_v3_exact_draft_and_discount_contract(db_session, monkeypatch):
    ...
    monkeypatch.setattr(web_agent_service, "recover_plan8_final_v5", fake_web)
...
def test_plan8_v3_busy_does_not_create_or_consume_claim(
    ...
    _patch_scope(db_session, monkeypatch)
    monkeypatch.setattr(
        web_agent_service, "recover_plan8_final_v5",
        lambda *_a, **_k: {
...
def test_plan8_v3_claimed_failure_never_reexecutes_and_readback_is_read_only(
    ...
    monkeypatch.setattr(web_agent_service, "recover_plan8_final_v5", first_web)
    ...
    monkeypatch.setattr(
        web_agent_service, "recover_plan8_final_v5", readback_web)
...
def test_plan8_v3_inspection_accepts_empty_activity_prices_before_full_patch(
    ...
    monkeypatch.setattr(
        web_agent_service, "recover_plan8_final_v5", empty_price_inspect)
    result = _run(db_session)
    assert result["ok"] is True
    assert result["verification"]["record_count"] == 6
    assert result["verification"]["sku_count"] == 78
...
def test_plan8_v3_inspection_rejects_sku_or_extra_record_before_claim(
    ...
    monkeypatch.setattr(
        web_agent_service, "recover_plan8_final_v5", bad_inspect)
    result = _run(db_session)
...
def test_web_agent_v3_busy_response_is_normalized_without_job(monkeypatch):
    ...
    result = web_agent_service.recover_plan8_final_v5(
...
def test_plan8_v3_cli_accepts_only_fixed_execute_or_readback(monkeypatch):
    valid = {
        ...
        "expected_status": "alarmed", "recovery_version": 5,
        "mode": "readback",
    ...
    script = Path(__file__).parents[2] / "scripts" / (
        "campaign_recover_plan8_final_v5_nas.ps1")
...
def test_plan8_v3_candidate_price_evidence_is_fresh_and_hard_gated(
    ...
    monkeypatch.setattr(web_agent_service, "recover_plan8_final_v5", stale)
    result = _run(db_session)
    assert result["error"] == "plan8_final_v3_inspection_blocked"
...
def test_plan8_v3_bound_drift_creates_no_attempt(db_session, monkeypatch):
    ...
    monkeypatch.setattr(web_agent_service, "recover_plan8_final_v5", drift)
    result = _run(db_session)
    assert result["error"] == "plan8_final_v3_inspection_blocked"
    assert db_session.execute(select(CampaignExecutionAttempt).where(
        CampaignExecutionAttempt.operation == recovery.OPERATION,
...
def test_plan8_v3_request_requires_explicit_mode_confirmation_and_scope(
    ...
    denied = recovery.recover_plan8_final_v3(
        ...
        expected_status="alarmed", recovery_version=5, mode="execute",
...
def test_plan8_v3_claim_verify_machine_identity_is_path_scoped(monkeypatch):
    ...
    path = dependencies.CAMPAIGN_PLAN8_FINAL_RECOVERY_V5_CLAIM_VERIFY_PATH
    assert dependencies.machine_identity_for_key(
        "secret", object(), path=path
    ) == "machine:web-agent-plan8-v5-claim-verify"
    assert dependencies.machine_identity_for_key(
