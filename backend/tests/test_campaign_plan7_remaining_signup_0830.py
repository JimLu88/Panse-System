...
def _rows():
    ...
    for item_index, item_id in enumerate(item_ids):
        item_row_count = 9 if item_index == len(item_ids) - 1 else 10
        for _ in range(item_row_count):
            global_index += 1
            is_placeholder = global_index <= service.EXPECTED_PLACEHOLDER_SKU_ROWS
            rows.append({
...
def _patch_scope(monkeypatch, *, terminal_failure=False):
    ...
    def fake_refresh(*_args, **_kwargs):
        ...
        accepted_rows = rows[:-1] if terminal_failure else rows
        return {
            "ok": True,
            "rows": [{
    ...
    monkeypatch.setattr(
        ...
        lambda *_args, **_kwargs: [
            {"rule": "R16", "level": "pass"},
            {"rule": "R17", "level": "pass",
             "checked": sum(not row["is_placeholder"] for row in rows)},
        ])
    monkeypatch.setattr(
    ...
    def fake_upload(*_args, **_kwargs):
        ...
        return {
            ...
            "validation": {
                "total_items": 30,
                "ok": 29 if terminal_failure else 30,
    ...
    def fake_classify(_db, _plan, _result, _pending, correct_items):
        accepted = set(service.AUTHORIZED_PLACEHOLDER_ITEM_IDS)
        hard = set()
        if terminal_failure:
