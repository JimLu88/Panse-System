...
def _source_file(asset_root: Path, value: str | None, fallback: str) -> Path:
    ...
    # manual_asset_index.json is produced on Windows.  pathlib.Path on the
    # Linux API container treats backslashes as ordinary characters, so use
    # PureWindowsPath for Windows-shaped source paths before joining the
    # caller-supplied import root.
