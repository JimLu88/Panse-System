...
## 2026-09-01 计划7小促价与仓库定制SKU一次性修正入口
...
- 仓库商品 `1038725569412` 的 SKU `6060112621275` 固定从 1500 改为 1420。
  14 行编辑器、仓库列表 10 列、库存 13006、标题和所有非目标字段全部锁死；商品必须
  “仓库中 → 仓库中”。程序只允许精确“放入仓库/仓库中”，绝不点“立刻上架”；未知
  弹窗或登录墙立即停住，不外发二维码、不自动重试。正式脚本为
  `scripts/campaign_correct_warehouse_product_sku_price_nas.ps1`。
- 02 只交付程序，不执行以上两个真实业务命令。完整规则见
