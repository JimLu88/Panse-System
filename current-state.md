...
## 2026-09-26 商品 SKU 原表版本索引与本机制表读取

- 新固定入口 `scripts/campaign_sku_fact_store.py query` 自动发现已登记最新原表；`register` 保存原字节/hash/导出时间/索引时间，`bind` 为旧ERP价格快照建立新只读绑定副本。详见 `docs/campaign-sku-fact-store.md`。新价格快照自动绑定最新版本，制表 `build_rows` 已读取精确 item+sku+merchant-code，不改ERP主映射/价格。
- 原件 `../活动准备/商品SKU事实/20260926-230536/0926 23-05-36 9760export.xlsx`，SHA256 bd323236c68538710cb609e6c6a3f24d2466183652ad2da66d717a9ed06c64e0。导出2026-09-26 23:05:36 +08，登记23:17:07 +08。94商品、854数据行，其中847条有SKU ID、7条无SKU；全表234行空商家编码（含7条无SKU行）。不是854个可报名SKU，也不证明全量在售/启用开关。
- 目标3件40SKU独立完整行交叉读回：32可精确对应；齐边床1038064128030的16条通过固定制表读取（12普通优惠行），保留ERP主链接1074244132390。餐边高柜3条代码含义冲突、曜黑柜5条编码空继续隔离；旧权威恢复记录仍将曜黑5条列未解，不按相似名称移用1036471324464。
- 程序只在本机读取和产出证据，不上传/报名、不启动/恢复旧任务、不重放成功或未知、不动订单/财务/库存/飞书。群晖API/Web无代码变更，无须部署重启。关系维护文档登记此本机链路，运行关系图仍明确未建模，未假挂无关后端锚点。
- 实际验证与安装回执：`../outputs/sku-fact-store-20260926/`；03继续本地制表，不因维护等待，用户自行上传。

## 2026-09-25 店铺口碑巡检

- 新增独立飞书提醒群通知接口、发送前持久去重与真实 message_id 回执；关系流程增加口碑采集、判断、通知及财务保护边界。订单、工厂表和评价素材流程不变。
- 配套 Web-Agent 固定程序覆盖中差评全部分页及全部问题的公开回答；每周二/五10:00，初次历史与新增分开，无变化静默，未知发送不重试。详见 docs/customer-feedback-patrol-20260925.md。
- 本地实读：中评1/差评2、78个问题/66条公开回答。代码提交不等于上线、外发或调度启用；最终部署与真实送达见项目 outputs/customer-feedback-patrol-20260925.md。
- 扩展回归的旧活动发现测试仍模拟旧 _post 路径，而原主分支实现走 continuous/observe；该测试失败与本次通知接口无关，未修改活动执行流程。关系目录保留另两处原有过期指纹，未伪装全库复核。

## 2026-09-20 多件/多商品事故修复（待正式发布回执）

- 活动暂停。用户样本3316819945093166090当前2件，原440单旧图BOM1套；共核实6张同型旧图数量不足。另5127637176073013917两条上/下柜购买行被“入户”子串服务过滤误排，旧410图不能证明实际发齐。
- 修复服务名称整词识别、同子单冲突阻断、明确数量展示、真实渲染数量/SKU/hash快照、完整性未知状态；单SKU木作估算按有效数量乘算，补明细/件数后刷新派生成本。
- docs/order-completeness-incident-20260920.md：计划及13类反例。126项相关回归通过；最终34项事故专项通过；生产只读prepare7单通过，正式引擎预览大字2件/BOM2套已视觉核对。不是部署或外发成功证明。
- order_completeness_incident.prepare/repair/notify为本次固定来源3027/hash与7单的限定恢复入口。补2条购买明细，不新建生产单；更正通知/8图防重认领，未知不重试；01负责正式ERP群外发。原消息、旧图、实际账单保留。
- 480档案按hash去重321份：196可读、114加密、11缺失；不能把源文件不足或旧子单号未迁移称为无遗漏。实际物流还需仓库逐件确认。

## 2026-09-16 最新：官方新成功已取得，剩余收尾程序修复

- 家具1001358847694：超级立减批865049608成功，秋季5SKU营销10031382708683保留。九斗柜793052650673：秋季批865078622成功；超级立减已有成功受保护，当前SKU/优惠范围待收尾。
- 修复固定导出radio空白/名称识别；成功兄弟导出复用；九斗保护先于优惠写入，缺优惠单列gaps而不冒称完成。价格/范围/底线及成功未知不重传规则未改。
- 原两请求只读prepare通过；ERP50项、WA44项测试通过。ERP bf5e569 / WA46297e2已push/install；本次空闲重启被工具执行前blocked by policy拒绝，未绕过。在线仍8500 PID3996、8502 PID40884、active=false，新收尾路由未加载。已让03暂停接续，需人工原服务窗口重启后核验，Edge保持。磁盘修复不等于加载或最终业务验收。
- 最新交接 D:/AI/畔色ERP系统/outputs/campaign-final-closeout-repair-20260916.md；此前“超级立减未成功”“手动重启未完成”等属于旧状态，不再代表当前。
...
- 家具两条新增首次定制原价13000/20000已经用户确认并登记，基线回执 docs/receipts/campaign-furniture-two-user-established-baselines-20260916.json（ERP权威目录）。不再重复询问，不随修正价重设。
- 固定程序从原模板身份/SHA证据恢复真实官方页，先录屏后导航，一次原凭证上传，终态自动接回原整段控制器。不得让日常AI逐job接续或自由控页；成功/未知不重传，价格/范围/窗口不变。
- 两个原请求417eb79d…（家具1001358847694）与0c3b394a…（九斗柜793052650673）尚无本轮新成功回执；必须执行后看官方终态，不能用本地测试、健康200或历史结果称全部报名完成。
- 最新维护记录：D:/AI/畔色ERP系统/outputs/campaign-prebinding-program-release-20260916.md。测试模拟不代替全自动无人值守验收，不伪造源码指纹刷新定时验收。
...
- 家具1001358847694主档已完成；超级861801345因需包邮失败，未获新包邮承诺；秋季828959968原报告已取回，3条固定原价来源仍缺，在线恢复HTTP409尚待确诊，原417eb仍未成功。不重下报告、不动原回执。
- 升降桌已发布新定制6301497267888接入原两表程序，12条ON/旧尺寸OFF真实证据核验；03一次启动cee9816972fe81447e99c34d9300e749508002e6dd2b6fea2c55df538eed606f，当前仅running，未称成功。秋季原成功保护优先，普通8条及所有价格/库存不改。
- 九斗柜秋季尚未完成，两个备用库存0，库存沿旧款的授权已向用户提出；差价链接排除。详细检查点 ../outputs/three-products-progress-20260915-1319.md。

## 2026-09-14 两场剩余 SKU 固定读取已实测，完整轮换接续仍未完成

- Web Agent aa659be/32d8b1c已push main并安装；新增绑定两场最终审计的product_final_audit_read，修复上架开关aria=1被识别为未知的问题。95项关联测试及后续31项回归通过。
- 秋季3商品34行、超级立减15商品126行已真实只读核验，未改价/发布/轮换/报名。秋季27帧0采帧错误；超级136帧1采帧错误；两份视频SHA均核验，旧原始回执不改。
- 两场窗口已由03确认：超级9月28日00:00—9月30日23:59:59；秋季9月16日20:00—9月27日23:59:59。最新用户允许本次必要SKU轮换，但仅失败范围，价格/基线/原订单映射/成功未知不重放不变。
- 8500 PID46588健康且空闲，Edge8502 PID38752未重启。当前验收fce50cc7690e06dd526a51a543b0072401e9ee42b539f8a457ffdd45bf8d85d3仅原报名能力；接口明确post_audit_rotation_publish_ready=false，不能用ready=true冒称完整轮换链已完成。
- 尚未交付新的完整连续请求：旧SKU槽位接口仅历史升降桌草稿；仍缺通用受限正式发布/物理SKU映射/限定补报分支。03暂勿重跑已成功书柜或已无动销岩板请求，不得改用AI自由控页。详见Web-Agent程序/docs/campaign-audit-remainder-20260914.md。
...
## 2026-09-14 已交付原优惠缺失SKU补入程序（现场终态待03）

- ERP 18a844e、Web-Agent 39d3de7已推main并逐文件安装本机；固定discount_include仅原秋季offer144956016253、09-16 20:00至09-27 23:59:59，云朵6普通SKU+普塔2普通SKU。完整商品级/SKU级优惠清单与not_enrolled证据、同一ERP价格版本、成功/未知保护及一次claim均必需；不创建重叠优惠，不修改既有普通价格、定制基线或轮换规则。
- 固定程序保存并逐SKU回读一致后才record，再由原控制器自动恢复这两件输入缺口；不手改DB、不重报成功商品。全ERP活动451项+119子测试通过，新增恢复测试后关联33项+14子测试通过；WA关联76项通过。程序交付不代表平台报名成功。
- 云朵尾分隔符映射已由77c9910依据完整59商品快照修复并登记，价格版本未变，无重复导出。两件长期超级立减只读核对完成：普塔6301271146899、升降桌6301497267888活动价为空；当前营销行异常/待审核，不能据空价断言全部异常根因，也未执行长期重新发布。
- 截至本节写入，秋季本轮新增30，合并13既有=43官方确认发布；8本场动销跳过、5待处理，历史保护升降桌不计当前成功。03执行新补入及原请求续报，实际新回执待回收。NAS未部署。

## 2026-09-14 秋季本轮新增30成功，剩余原因已定位

- 03原request本轮新增30件成功；13件既有发布+30=43件，另升降桌历史保护但当前异常，不能把控制器success44说成44当前发布。8件本场动销跳过，5件未完成：云朵6SKU/普塔2SKU实际未加入原秋季优惠（云朵另有已证实尾分隔符映射未继承）、岩板5SKU当前目标差超2、书柜官方文案截断、斗柜历史标价且无新有效改动。
- 6f2b4d7已安装push，零金额变化不再建立修改claim或将changed记true；全活动441项+119子测试通过。原两商品固定只读ca3d63…8087已证明8条not_enrolled，非缓存缺记录；不新建重叠优惠或复用已禁用旧plan7。
- 原28只读已真实完成并恢复上传，855891764为26/28，855892099为2/2；之后岛台成功。详见docs/receipts/autumn-signup-20260914.md。没有本轮NAS部署、清除成功保护、自动轮换或改变冻结规则，尚未全量报名成功。
...
- ddefb77固定区分精确分段的既有无动销失败与另一分段历史报告；82项+6子测试通过。相同长期活动ID不是永久排除依据；本次已产生新官方动销失败，按本轮跳过。
- Web Agent2501843修复空导入框挡住通知的上传前点击超时，原job一次接续完成；真实失败报告自动下载，官方结果/录像/文件哈希均由02核验。不是所有商品报名成功；NAS版本仍038bd0e，CLI程序已本机安装。
...
- 本轮11件40SKU确认为实际下架后只从报名文件排除；另2件已建B1备用SKU别名核实并补报成功。70条单品立减真实成功且金额/9月28日至30日窗口读回一致，已成功不重传。
- ERP fed6239、Web Agent e0212b5已推送并安装，11个最新运行文件SHA256一致；ERP396项+119子测试通过，WA全活动409通过后旧文件名测试修正、关联21项通过。固定流程由03执行，不切换AI浏览器手工控制。
- 定时PAUSED，历史录屏3次及后续读回1次采帧超时保留，不冒充全流程录屏验收通过。4商品5条新缺基线未继承此前3商品授权；详细依据、例外与人工决定见docs/receipts/campaign-batch-mapping-fix-20260913.md。

## 2026-09-12 round37最终27成功，云朵编码解除后本场跳过

- 当前complete/round37/pending空：57=27成功+15本场无动销分组+11映射（当前40SKU）+1升降桌轮换+3价格例外。不是全部成功；mapping_summary自动区分历史66与当前40，不修改历史台账。
- 云朵36精确旧编码别名已解除冲突并实际提交，平台返回动销准入及历史价格混合失败，按既定本场无动销规则跳过，不继续降价/重传。仍缺的11件映射事实与3价格例外没有伪称解决，20%保护线和暂停定时保持。

## 2026-09-12 round35已27成功，云朵36历史编码单件恢复

- ERPd56ca74/WA3703810已安装推送；云朵36条精确物理SKU/规格对应仅补历史无PPS前缀别名，不改变价格/身份；03唯一恢复此商品，原成功不重传。ERP371项+107子测试、WA6专项通过。
- round35互斥57=27成功+14无动销+11映射+1云朵编码+1升降桌轮换+3价格例外。后续云朵终态仍待03，不能假称28成功。11映射历史66条、当前40条已分开输出，历史台账不改。
- 原6优惠误挡已5成功，余722275846168平台要求报名价<3036但ERP日常3472.50；不自行改日常价。原3基线已2无动销、1升降桌低于400底线需轮换。详见25件排查回执最新顶部。

## 2026-09-12 25件缺口再排查：已修定制误挡/退役登记，非全部轮换

- 本轮最终程序发布ERP0c50f08/WA3060f43。新145807488351导入60成功15无效SKU失败；固定只读job9dcd8d27…4283f02e已完成60金额0差异/窗口一致/100帧19事件0捕获错误，03唯一apply后恢复原controller。原20/268不重传，新增活动成功尚待后续终态，不能宣称14映射全部修完。

- 用户明确授权3条缺失基线以当前定制价固定；新增1333.33/2000/5000，按分底线266.67/400/1000。已有基线不覆盖，当前价格版本不改。
- 14商品78SKU缺口中37条属于ERP现行下架登记漏接，另1条云朵沙发商家编码多余竖线已精确补对应；3商品映射阻挡解除，11商品40SKU仍需状态或价格数据证据，未强行恢复旧轮换ID。
- 6商品21条定制SKU原未进入普通单品立减，却被商品级部分导入unknown误挡；以原提交/失败/成功读回精确范围修正，不解锁原268成功优惠或旧未知写入。
- 新固定recover-data-gaps已安装，03接纳9条重新生成范围+3条基线重分类；新官方结果尚待原controller回执，不宣称新增成功。365项ERP测试及107子测试、9项WA专项通过。
- 详情：docs/receipts/活动25件缺口排查修复-20260912.md。旧57范围快照保留，定时PAUSED/完整录屏未验收状态不变。

## 2026-09-12 四类规则固化收口：20成功，剩余按证据分类

- 用户四项规则已实现、安装并推送主分支：ERP业务代码edffe01、Web Agent业务代码8466c5d；19个安装文件与源码逐项一致。
- 03原连续任务最终round27、pending空、无在途动作；57商品互斥为20官方成功、12本场无动销、14ERP映射缺口、6物理SKU优惠unknown、3原定制基线缺证、1叠加价格未解释、1修正无效果。
- 餐边柜1007407909979/6228006543290实际立减最终3102.13；三次报告券后价未变化。已修复无效果仍重复让价的缺口，后续3102.23在改价前被拦，未派发claim已固定失败收口，不释放不重试。
- 2商品5条备用编码完成精确映射登记，价格版本不变，但其余SKU优惠仍unknown，不据此宣称新报名成功。
- ERP全报名319项与107子测试通过；WA全报名362项、后续字段/恢复专项32项、无效果与别名保护10项通过。20成功官方回执与46视频实文件哈希复核全部一致。
- 业务未全部成功、全录像验收未通过，定时任务保持暂停。原268成功立减及20成功商品不重传，不自动轮换、不重新全量导出。
- 详细结果：ERP程序/docs/receipts/活动四类规则固化及报名结果-20260912.md；结构化程序回执同目录campaign-four-rules-release-20260912.json。


## 2026-09-12 四类失败固化、保留部分成功与小额修复

- 本场动销失败跳过；无效SKU由固定完整导出核对后仅从报名文件剔除；定制按首次原价20%底线自动下调；普通同一目标价2元内自动修复，超过进入轮换待确认。
- 268条成功优惠按原失败文件与完整SKU读回交叉证明后复用，商品级未知仍保留；不重传成功、不替未知SKU补造成功。
- 小额券后口径不一致时，计算及所有官方观察值在调整前后均须处于原ERP目标±2元；不猜券、不追加额度、不改ERP日常价。
- 固定恢复原未派发改价任务需精确原job、未消耗claim及操作证据；搜索字段先失焦提交，再按2.5秒节奏点击，保存前仍消费一次性claim。
- 程序本地安装，ERP全报名回归317项+107子测试通过；WA全报名回归362项通过，后续字段/恢复专项另验。定时/全流程验收未解锁。
- 当前平台去重20商品成功；原位小额修复及2商品5条已核验备用编码补充仍待后续终态，不得据此宣称全部报名完成。


## 2026-09-12 完整导出复用与备用编码精确对应

- 固定导出512654已完成59在售商品/3页/666SKU，页与文件覆盖一致；录制187帧16事件0错误。
- 已确认去重16商品报名成功（新增842644513的837902729785）；原268立减成功不重传。
- 同任务无商品编辑时，后续无效SKU报告复用并重验原完整导出，不重建下载；跨批次剔除取并集。
- 4条B1编码通过旧创建/保存回读、已登记ERP映射及本次导出验证，只补别名，不改变价格版本。
- 新报告已内嵌反馈时直接采用原终态，不能再寻找不存在的独立反馈job。3件映射已接回原controller；
  2件别名使用固定recover-mapping-aliases在空闲终态后接回，不允许日常AI直接操控浏览器。
- ERP针对回归71通过及20子测试，WA恢复保护12通过；剩余需平台终态，仍不是全量成功或定时验收通过。

## 2026-09-12 四类处理真实报名与SKU级防重复修复

- 官方841745016返回14/20成功，841800534返回1/2成功；去重15商品成功，不重传。
- 后续报告点名同商品另一物理SKU，旧签名仅含价格误拦两商品。现签名含SKU+修正内容，
  保留旧SKU防重放；1037237545657/1038071596128已通过固定入口接续，不扩大轮换授权。
- 完整导出目前遇淘宝记录页系统错误，失败不当空商品。关联记录页录像纳入验收。
  37控制器/录像测试与14子测试通过；还有映射/基线/轮换异常，不是全成功，不解锁定时。

## 2026-09-12 四类失败自动处理补齐

- 841060046反馈已通过固定程序自动取回，37商品/318SKU，官方0成功/37失败；不再要求人工下载。
- 新解析识别动销、无效/下架SKU、严格审核通过价及带字段名前缀的属性；定制20%内直接降价，
  普通目标±2元内原位修正，超界轮换仍需确认。原价基线不重设，不改旧批次指纹。
- 固定全商品录屏导出→ID/唯一编码核对→失败SKU文件去除→仅失败商品重新生成已接线；
  新ID仅作有原文件证据的别名，不改ERP价格版本/数据库。原成功268不重传。
- 独立反馈恢复入口仅重开原report读取断点，报名终态不可重放。当前仍需03实际运行验证，
  不能把本地测试、自动取报告或代码交付称为报名成功。

## 2026-09-12 失败录屏与处理归属汇总

- 用户要求失败后继续录屏，由固定程序判断安全重试或交人工。Web Agent仅为明确
  终态的同批失败报告提供一次安全重下，报名/改价/SKU规则及未知写入防重放不变。
- ERP最终结果增加 failure_handling，携带原作业、处理归属、报告重试次数和录像。
  无视频、失败后缺帧、录屏提前到时均不能通过完整录屏验收，不抹掉官方成功回执。
- 当前原单品立减268成功/15失败不变，原报告仍缺，活动报名尚未开始；不是全流程验收通过。

## 2026-09-11 晚 连续报名具体运输与受控运行入口（真实验收未完成）

- 已接真实 ERP 快照、一次商品导出映射、固定/本场模板、同版本两表、分段优惠、
  一次性上传凭证、原始报告归一化、小额优惠原位修改和保存读回到连续控制器。
- Web-Agent 接发现登记、派发、精确请求状态、单次验收及验收封存接口；按需唤醒，
  只有两种真实平台终态及完整录屏绑定当前代码版本后，定时入口才可 ready=true。
- 固定母版不能作为当日报名状态；新模板必须整个商品各 SKU 均成功才跳过已报商品。
  有基线/输入问题的商品集中保留异常，其余继续；旧成功/未知批次不重传。
- 真实官方长期页已恢复可见；本次唯一生成的母版为
  `「超级立减长期活动」商品导入模版20260911231015.xlsx`。文件名按钮未返回下载事件，
  已请用户点击右侧下载图标交接文件；不重复生成，不将部分录屏当作全流程完成。
- 仍未取得真实全流程验收。单品立减部分失败报告的实际格式、固定母版配合新 SKU 的
  全范围更新、发现任务到新入口的定时验收，须继续用真实平台证据核验，不能宣布全自动上线。
- 发布及本机安装状态以后续项目 outputs/campaign-continuous 发布回执为准；本节不是部署证明。

## 2026-09-11 单品立减创建回执接线；全流程仍未完成

- 新增单品立减 claim 的权威 SKU/金额输出，Web-Agent 只能使用该确切范围。
- 新增 `campaign_discount_receipt`：创建汇总成功之后，必须核对实际优惠 ID、精确时间、
  全 SKU 金额，再写原有逐商品终态。读回失败保留未知，不能重传。
- 244 项 ERP 活动脚本测试、23 项本次 Web-Agent 目标测试通过。没有真实上传/改价/报名。
- 用户明确允许本次从官方页面重新下载“超级立减长期活动报名模板”并固定新母版；
  这不是单品立减模板重新下载授权。原单品立减母版及 SHA 保持不变。
- 已点官方固定页“查看详情”，实际打开 apply.htm?signRecordId=3172207691，只有千牛菜单，
  无活动内容/下载控件。官方日历也回到固定页；不猜地址、不重复下载、不伪造模板。
  项目 71 份文件无旧母版 SHA 命中，桌面/下载 43 个候选中 42 个可读文件无命中，
  1 个正在编辑的工作簿未强行访问。已请用户查看官方页面或协助下载。
- 尚缺完整生产控制器 adapter、旧版超级立减页面传输、修复执行接线、全链路录屏及
  03 的真实业务验收；不能把上述组件测试或缺模板当成“其他全部完成”。
  未合并 main、未正式部署，业务 ready=false；本次成果保存在维护分支。

## 2026-09-11 商品导出与优惠读回接线检查点

- `campaign_product_scope.from_edge_job`已接专用Edge全页导出回执，核对店铺、请求、
  页数/商品数、导出记录ID、文件SHA和逐页实际覆盖；库存不推断SKU上架状态。
- `campaign_discount_readback.verify`核对精确优惠ID、商品/SKU、时间与金额；
  允许明确的零立减，不允许负数/未知/漏行/重复/跨窗口冒充成功；仅报告差异，不改价。
- 全商品导出客户端总等待上限支持多页独立任务；只提交一次，后续只读同一个job。
- 真实Edge修复了优惠页控件重名、固定列遮挡及SKU金额定位，已只读得到两条金额。
  这是组件证据，不是已完成报名或整个adapter验收。完整生产接线仍未完成，ready=false。
- 241项ERP活动脚本测试、33项Web-Agent相关测试通过。
- 分段时间沿用用户已确认合同，不重新讨论、不变更旧规则。此次无淘宝上传/保存，
  无主分支合并/NAS部署，不要求03提前报名。见docs/campaign-continuous-flow-20260911.md。

## 2026-09-11 用户确认分段设置时间：本地生成/凭证/控制器已接入

- 日常空档M、大促精确窗口B；官方长期有效期与优惠时段分离，不建至2028年的长期优惠。
- 新独立时间规则65f80517…，旧500d344d…/7ff08aaa…及旧批次语义不变。
- 生成器锁定计划输入与同一ERP版本；优惠claim使用分段时间，报名claim使用官方有效期。
  已报名的日常商品仍补新优惠段，但不再报名；未知/成功旧优惠不得自动裁短或重放。
- 新24项隔离测试，活动脚本230项通过。无真实报名/改价，未主分支合并或NAS部署。
  完整adapter、全链路录屏与03业务验收尚未完成；详情docs/campaign-segmented-time-20260911.md。

## 2026-09-11 连续报名程序维护：报告与异步传输已补齐，未整体上线

- 206项活动脚本、18项后台目标测试通过；新增全量官方报告解析、精确SKU约束合并、
  全商品文件覆盖验证、独立Edge任务客户端、官方终态回填和上传前一次性凭证消耗。
- 已知活动截止提前检查已接到72小时调度；新global规则显式选择，旧claim指纹不重释。
- 用户已登录专用Edge，真实表单秒级窗口/批量导入/SKU级无上传验证通过；录屏取帧修复后
  99帧真实MP4可读。0次上传、0次提交。全部流程录屏及真正业务验收仍未完成。
- 尚未连接完整生产controller adapter，未合并主分支/未NAS部署/未安排03提前报名。
  待办与证据见docs/campaign-continuous-flow-20260911.md。02继续只做程序，03最终测试。

## 2026-09-11 连续Web-Agent报名：核心检查点，未上线

- 用户批准固定myseller入口、ERP可售∩淘宝上架、AI只找链接/辨新活动/经人工确认轮换；
  其他连续Web-Agent执行。新增版本化规则及持久控制器，不改旧冻结指纹、批次、固定基线。
- 新自动入口停止调用旧预检链；运输未接通明确blocked。每72小时发现有持久尝试时间门。
- 用户已同意专用Edge：真实独立窗口及本地文件选择测试通过，当前停在淘宝首次登录门。
  真正淘宝上传及连续执行适配仍未验证，旧浏览器与82条固定基线不动。
  未NAS发布、未修改生产定时、未实际报名；不能声称已全自动。剩余缺口见
  docs/campaign-continuous-flow-20260911.md，测试中的FakeWebAgent不能当实际适配器。

## 2026-09-11 用户建立78条定制固定基线及精度更正

- 本次78条以用户当前明确授权建立固定基线，不冒称历史首次价；此前4条原样保留并加入默认来源。初版2条floor108.33错误，v2保留原价541.67、改精确floor108.334，可提交到分最低108.34。
- 正式correct_fixed_floor_source已执行精确替代，旧文件/登记保留审计，新读取只用v2；新fixed登记先验精确计算，防止再次污染。原价、范围、其他76条和已有4条不改，不解锁报名claim。
- 新进程独立验证78+4均加载，82条最低分价允许/低0.01拒绝、降低当前价不重算基线；152项活动测试通过。本地来源修复与固化，无平台改价/报名、无NAS部署。已确认批号和78基线不再询问。详见docs/campaign-user-established-baselines-20260911.md。

## 2026-09-11 本场失败普通SKU原位立减修正入口

- 旧计划7固定4条补入入口不能用于本场。新增本地campaign_discount_amend claim/record及run_once回调合同，精确绑定本场优惠144956016253、失败claim、原报告、授权、普通SKU与旧/新金额；不新建优惠、不整批重传，不改daily/定制。
- 原子本地占位后unknown防重放；必须实际旧值匹配，成功后新金额读回及终态证据才覆盖相应行。原成功文件保留，其他行不覆盖；生成与提交统一消费覆盖层。不是平台原子CAS，03同一页面旧值比较和单写入所有者仍必要。
- 146项隔离测试通过（含并发单写入）；无真实改价、平台写入或NAS部署。交付时31失败claim仍unknown（待官方批号），本场原位编辑页面/运输能力未确认。程序入口可交付，但不能声称这9条已修改或报名成功。详见docs/campaign-discount-amend-20260911.md。

## 2026-09-11 秋季本场正负2元最终价容差

- 用户本轮明确授权两元以内放行，由03转02实现。只覆盖49557/49560/3538210379、9月16日20:00至27日23:59:59、12%及大促目标；不引用旧52行正向授权扩范围。
- 正式生成、validated_body及原子claim共同校验字节锁定授权，使用真实已成功立减，逐SKU绝对差<=2.00含等号；其他金额/窗口/成功保护及定制固定20%底线不变。不得主动全部减2元。
- 本地入口修复，无NAS部署/ERP改价/成功优惠修改/平台上传。详细合同及验证命令：docs/campaign-autumn-two-yuan-20260911.md。

## 2026-09-10 活动本地统一入口与实际优惠复用守卫

- 本地生成器/价格快照自动读持久登记映射、固定原价及历史终态；受控claim/record或run_once在同一SQLite先占位再写入，成功/未知不重放。普通daily及首次原价20%规则不变，缺基准不拦首次不降价。
- 实际复用优惠按已成功上传文件金额复算，不能用新理想deduct代替。保存的秋季144956016253成功证据绑定57商品398行原文件；对03本次34商品297SKU分析复算188行/31商品低于大促底价0.02–0.94元，22普通行精确相等。历史导入不是当前平台读回，未修改成功优惠、价格或报名。
- 范围/入口/测试见docs/campaign-entry-guards.md。原始浏览器按钮、直接底层函数、旧NAS活动API/定时任务仍是未接入旁路，不声称已经全局拦截。当前03须消费新claim；02不接管浏览器、不唤醒00。仅本地文件代码，不部署NAS服务。

## 2026-09-10 工厂同步分页和来源清空修复（待正式上线验收）

- 修复异常分页缺token/重复token，记录ID及页数有界；不接受部分表冒充全表。
- 0154只增可空来源JSON，无历史清空。平台买家/商家备注和物流来源列/一致值/时间版本/原值CAS贯穿现有agent导入；未知空值与人工值保留，物流撤销额外要求可信状态反转并在飞书再次CAS。
- 新固定回归、来源保护及seed2026091002的10独立生命周期合成单已执行；原20反例+117旧例137通过，新增来源保护15+随机10通过。正式完整回归和线上/01真实随机10单分别验收，不把本地测试当完成。
- 详见docs/factory-sync-source-provenance-20260910.md；不改活动规则、价格、6小时调度，不批清历史。

## 2026-09-09 平台备注标签与人工发货月份分源修复

- 正常淘宝导入持久保存备注标签原文与文件指纹来源，修复“主订单买家留言”别名。缺列保留，显式全空清平台标签；同单多行非空合并并提示冲突，不覆盖人工备注/月确认。
- 发货延期与生产权限分开：平台标签不触发开工；已有人工作业身份的两单按用户确认保留生产，379为2026-10、375为2026-12，只存月份，具体日待确认，不按月初/月末自动发货。原备注和工厂身份不变。
- 工厂表沿用现有列及6小时同步，月份/来源显示在订单备注与提醒，发货安排沿用“做好后等通知发货”，清除误导的有效日截止；终态优先，图片和人工列保留。ERP卡片/导出/打印同步展示，旧日期编辑器不能覆盖月确认。
- 117项相关测试通过。部署与两单实际补同步须以outputs正式回执为准，代码状态不是外部完成。01精确执行入口及边界见 docs/factory-shipping-note-sources-20260909.md。

## 2026-09-09 工厂飞书表全字段增量同步修复

- 生产只读查实：每6小时执行且最近8次成功，但子订单状态硬编码生产中导致终态错配。截图392/379/360源paid无发货事实保持生产中；298/397及实际139行子signed按真实子状态处理。
- 修复子状态/交期/发货安排一致性，精确SKU补规格，保护未知来源成本与人工列；缺图不清已有附件，取消退款原位标记作废而不删除，未知记录保留。
- 明确字段白名单与差异写入，进程/数据库单写锁，写后字段回读；默认只读CLI支持01前后全字段验收，不迁移/重建表。
- 字段合同、空值语义与10项正反证据：docs/factory-dispatch-field-contract-20260909.md。当前代码提交不是已部署/补同步证明，实际部署与01正式执行回执另存项目outputs。

## 2026-09-09 单品立减固定模板复用

- 用户明确：单品立减复用1788924257335.xlsx，不再要求人工/AI重复下载；活动报名继续当次最新模板。固定母版及指纹见冻结合同single_discount_template，仓库保留字节相同官方副本。
- 两表生成器默认本地固定单品母版，拒绝指纹不同的旧填表/错误文件，每次清除示例数据只填本次SKU和金额；母版不改。价格、窗口、成功保护和活动模板规则未改。
- 本次仅本地生成工具/文档，无浏览器、平台或数据库写入，无NAS服务部署必要，不阻挡01当前业务。

## 2026-09-08 定制失败依据链与误判轮换防错

- 替换旧仅两来源且固定覆盖11条的临时输出脚本，显式版本化来源目录、逐行状态和共用固定回执适配器进入权威scripts。null不代表无价格或必须轮换；历史daily不自动成为首次固定基准。
- 原A组45加已保存餐桌2为47条核对集合/57活动分支；中间46已撤回，升降桌超级立减失败不能被其他活动成功跨场排除。新直接可执行0，已保存待回读2；47条已有历史价格但固定身份待证，不交为必换清单。已有11条成功固定基准保持保护。
- 定制本场可行申报价>=首次固定原价20%含等号，按已有授权直接修正优先；修改仅限本场申报价/精确优惠窗口，不改全局商品字段或影响其他成功活动。真实备用优先复用，已保存不得重复建立。
- 本地工具修复，无业务数据库/浏览器/NAS服务变更。详细来源、范围与验证见docs/campaign-fixed-basis-resolution.md；报名是否成功由01官方回执另验。

## 2026-09-06 23:20 秋季5商品补报官方成功（当前最新断点）

- 精确秋季49557/49560/sign3538210379，官方批次801000629，23:20:08终态5商品成功、0失败、0待查看；完整62SKU上传一次，5商品逐个“已发布设定”回读。全场当前已发布12/草稿1，草稿793084818113；该批及5商品禁止重传。
- 5商品：1036279566778、1074244132390、841201084787、837902729785、1036312802226。复用已成功单品立减144956016253，无优惠重传。46普通价未改，16定制中11条按现有授权调整；40普通容差为精确已有授权，不开放未来任意降价或轮换。
- 当前已授权可办理范围完成，不等于全店全部报名成功。秋季原失败剩45=8本场无动销+37价格未解；超级立减原30失败本轮未新增提交=8动销+22价格，原28活动中保护。
- 82条定制基准未证实（含1分类疑点）继续跳过，不能写成82商品；原失败批外另3商品31SKU缺映射。逐SKU官方到手价未导出，只核验整品官方发布及页面价范围。
- 同Edge新受控页恢复已经通过本次真实文件选择/单次上传/官方终态链验证。模板下载事件曾超时但文件实际落盘，未重复点击；不得以旧连接失败或5商品“待授权”继续阻挡，也不得称底层旧页附着故障已修好。
- 状态回执：`docs/receipts/campaign-autumn-five-official-success-20260906.json`。原生成阶段receipt.json的local_file_ready_not_uploaded是历史阶段，平台事实以其后execution.json为准。仅固化回执与状态，无代码/价格规则改动，无NAS部署。

## 2026-09-06 用户直接要求固化定制备用与单一协调入口

- 原价格体系与两表短流程保持不变；02 对接用户并维护程序，01 唯一业务/浏览器执行。固定合同与 AGENTS 同步此分工。
- 新增离线 campaign_reserve_policy：区分直接价格修正、复用备用、确认缺口、基准/库存未知及普通/分类疑点；建议不是写入权限，不做扫描或轮询。灰色下架并保留库存；未知不当缺少。
- 两表生成器共用固定首次原价 20% 校验；真实旧新映射回执可传递固定基准、来源及链，拒绝跨 ERP 编码、链断裂、循环与重算底线。
- 68 项活动本地测试在独立工作树及主目录均通过；未改业务数据库、未部署 NAS、未触碰浏览器或已成功活动。固化完成不代表备用已建或剩余报名成功。
- 一次性补齐仅限当前超级立减/秋季明确失败范围内确认必要的定制备用；未开放未来自动轮换、普通轮换或撤回。规则改变仍需用户直接指令。详情见 docs/campaign-signup-frozen-steps.md 与 docs/campaign-current-file-tools.md。
...
- 当次官方模板 20260906050104 含这 4 商品全部 38 个启用 SKU；28 普通沿用 ERP 日常价与立减 144226431384，10 定制在固定保护价内填写。仅上传一次，批次 **795728875**（05:13:50），官方终态 **4 成功 / 0 失败 / 0 待查看**。
- 完整刷新活动页后：**全部 51 / 已发布设定 51 / 草稿 0**。两床 32SKU 成功批次 795099381 及本次 795728875 均禁止重传。当前剩余商品报名已清零，不再执行旧 52-only、6 草稿或 4 草稿准备包。
- 本场售卖窗口 2026-09-06 20:00:00 至 2026-09-13 23:59:59。报名完成不等于当前已经开售；也不等于下文历史全店最终价审计的所有不确定项已解决。可后补新颜色图仍未补，不阻挡此次官方报名成功。
- 未来继续 **01 与用户共同报名、02 维护**；本次临时接手已结束，冻结规则没有修改，浏览器保留打开。以下早先未完成记录仅为历史，以本段及最新官方回执为准。

## 2026-09-06 固化收尾及本场全部SKU失败终态

- 用户明确未来与01共同报名：01做文件和平台操作，技术下载/上传约3分钟失败即给精确页面和文件，请用户协助；02只维护，不抢浏览器。已写入固定规则和离线回归。
- 批次795099195已终态0成功/6失败，官方要求全部SKU，缺18定制。52-only入口永久封口，不能再称普通范围可独立推进。该事实只用于本场失败后续办，不增加常规预检。
- 新一轮01授权范围为两整商品1074244132390、1036279566778共32SKU：24普通沿用已验价格及立减，8定制按官方建议444/285且固定底价200。文件生成不是报名成功，终态由01记录；其他4商品价格硬冲突独立保留。
- 本次为本地文件工具、规则及回归固化，不需要NAS部署/服务重启；不宣称旧运行时已经替换成端到端短流程。

## 2026-09-06 计划8当前52普通SKU文件生成短入口

- `scripts/campaign_prepare_draft52.py` / `docs/campaign-draft52-short-entry.md`：
  只读一次ERP价格版本，复用已验证同窗口同金额的144226431384，生成52普通SKU活动表。
- 本场最新官方模板20260906020807，70草稿行；18定制原始基准仍阻塞。
  用户当前允许本轮正向到手价偏差≤2元，规则范围严格限于本轮52普通SKU；不修改全局冻结规则。
- 模板修复同时重映射数据行和合并范围、迁移被删锚点值，保留其他ZIP部件原字节。
  读取官方导出按真实数据行而不是错误dimension=A1。
- 该入口不上传、不提交、不改价、不重传立减或24已成功商品，不部署NAS/重启Web-Agent。
  成品仅交给01单次上传，平台终态与18定制未完成项继续分别记账。
- 01于02:22:11已上传唯一成品（SHA256 `139ede27d6e9ad1862ee8ae2562a34dbf176495adc4d4159603a1fae1a7768e2`），
  检查点当时为处理中；本次生成入口现已封口拒绝重放。后续以01官方终态回执为准。
