 ...
 * 推演打开页面即自动算入理论成本(无需逐单写回); 工厂实际成本到位后自动覆盖。用户拍板 2026-06-17。
 */
...
import {
  CustomReconcileRow, aiRecomputeCustom, fetchCustomReconcile,
...
export default function CustomReconcilePage() {
  ...
  const cols: ColumnsType<CustomReconcileRow> = [
        ...
        ? <Text type="secondary">未填</Text> : <Tag color="green">{yuan(v)}</Tag> },
    {
      ...
      render: (_: unknown, r) => {
        if (r.actual_cost != null) return <Tag color="green">工厂成本已覆盖</Tag>;
        if (r.needs_review) return <Tag color="red">待人工核价</Tag>;
        return <Tag color="blue">推演已入账</Tag>;
      },
    },
  ];
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      <Alert
        ...
        message="分级混合推演 — 打开页面即自动算入, 工厂实际成本到位后自动覆盖"
        description={
          <Paragraph style={{ marginBottom: 0 }}>
            规则先算: <b>写明成本/百分比</b>→直接取；<b>插座</b>→{data?.socket_material_code ?? 'AC-1007'}；
            都不中 → <b style={{ color: '#cf1322' }}>实付×85% 兜底(标红, 待人工/工厂价覆盖)</b>。
            复杂的（改尺寸/材质）点「<b>AI 重算兜底(后台写回)</b>」交本地大模型估算并写回(几分钟, 完成后刷新)；PC/Ollama 没开会飞书报警并维持 85%。
            <b>推演是预算价, 已自动写入理论成本喂给利润核算</b>，无需逐单点写回；实际工厂成本填入后自动覆盖它。
          </Paragraph>
        }
      />
