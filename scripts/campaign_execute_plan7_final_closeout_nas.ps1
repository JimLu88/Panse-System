...
# All fields are fixed. The CLI and service both reject any changed bundle,
# workflow, plan, status, policy, manifest, source or item-scope identity.
$payload = [ordered]@{
...
$remote = 'sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_execute_plan7_final_closeout'
...
try {
    ...
    if ($exitCode -ne 0) {
        ...
        throw "计划7最终收口 CLI 失败，退出码 $exitCode$responseJson"
    }
} finally {
