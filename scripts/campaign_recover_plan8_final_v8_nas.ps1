...
$payload = [ordered]@{
    ...
    confirmation = if ($ReadbackOnly) {
        'READBACK_ONLY_PLAN8_V8_NO_PLATFORM_WRITE'
    } else {
        'EXECUTE_ONCE_PLAN8_V8_RESUME_V7_ZERO_WRITE'
    }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Plan 8 V8 command failed with exit code $LASTEXITCODE. If a V8 write claim exists, use only -ReadbackOnly; never run -ExecuteOnce again."
    }
} finally {
