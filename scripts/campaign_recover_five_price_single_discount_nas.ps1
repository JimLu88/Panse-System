...
$remote = "sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_recover_five_price_single_discount"
...
try {
    ...
    if ($exitCode -ne 0) {
        throw "五项价格修正单品立减恢复失败，退出码 $exitCode。禁止自动重跑。"
    }
} finally {
