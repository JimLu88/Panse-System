...
$payload = [ordered]@{
    ...
    recovery_version = 8
    mode = 'resume_claimed_preupload_v7'
    confirmation = 'RESUME_ONCE_PLAN8_V8_AFTER_PREWRITE_BUSY_V7'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Plan 8 V8 claimed-preupload V7 stopped with exit code $LASTEXITCODE. Do not run this command again; inspect the returned JSON."
    }
} finally {
