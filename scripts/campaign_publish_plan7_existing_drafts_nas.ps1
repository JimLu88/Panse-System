...
$payload = [ordered]@{
    ...
    expected_scope_sha256 = '0355c293c277330e490858df4f6b4bb57484881fcea9897f27c194b68fb7231b'
}
$raw = $payload | ConvertTo-Json -Compress
$remote = 'sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_publish_plan7_existing_drafts'
...
try {
    ...
    if ($exitCode -ne 0) {
        throw "计划7两件既有草稿原位发布失败，退出码 $exitCode；禁止重跑"
    }
} finally {
