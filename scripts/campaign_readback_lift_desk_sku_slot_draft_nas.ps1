...
$remote = 'sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_readback_lift_desk_sku_slot_draft'
& $ssh -i $resolvedKey -o BatchMode=yes -o ConnectTimeout=20 -p $SshPort $SshHost $remote
if ($LASTEXITCODE -ne 0) { throw "升降桌轮换 SKU 草稿只读回读未完成，退出码 $LASTEXITCODE" }
