...
def calculate(snapshot, read):
    ...
    if snapshot['resolved_price_version_sha256'] != VERSION:
        raise ValueError('cost_revision_price_version_changed')
    if (read.get('state')!='readback' or read.get('platform_write') is not False
            or read.get('shop_name')!='畔色木作' or read.get('price_window')!=WINDOW
            or len(read.get('rows',[]))!=1):
        raise ValueError('cost_revision_exact_read_required')
    group=read['rows'][0]
    if (group['item']!=ITEM or group['window'].get('offer_id')!=OFFER
            or any(group['window'].get(k)!=v for k,v in WINDOW.items())
            or set(group['values'])!=set(SKUS) or set(group.get('not_enrolled',[]))!=EXCLUDED
            or group.get('requested_scope_verified') is not True):
        raise ValueError('cost_revision_read_scope_changed')
    identities=[{'item':ITEM,'sku':s,'state':''} for s in SKUS]
    activity, discounts, issues=build_rows(snapshot,identities,Decimal('.1'),'medium',{})
    ...
    for row in discounts:
        ...
        rows.append(dict(row,offer_id=OFFER,old_deduct=str(old),new_deduct=row['deduct']))
    return rows
...
def check_current(a,body):
    ...
    if blocked.get(ITEM)=='unknown':raise ValueError('cost_revision_unknown_signup_protected')
    offers=a.discount_offers()
    matching=[o for o in offers if o.get('platform_offer_id',o['offer_id'])==OFFER
              and (o['start'],o['end'])==(WINDOW['start'],WINDOW['end'])]
    if len(matching)!=1:raise ValueError('cost_revision_registered_offer_not_unique')
    offer=matching[0]
    if not any(i['item']==ITEM and i['status']=='success' for i in offer['items']):
        raise ValueError('cost_revision_existing_offer_not_verified')
    for other in offers:
        if other is offer:continue
        if other['start']<=WINDOW['end'] and WINDOW['start']<=other['end'] and any(
                i['item']==ITEM and i['status'] in ('success','unknown') for i in other['items']):
            raise ValueError('cost_revision_overlapping_offer')
    for row in body['rows']:
        saved=[r for r in offer['rows'] if (r['item'],r['sku'])==(ITEM,row['sku'])]
...
def claim(a):
    table(a);body=derive();cid=digest(body)[:32]
    a.db.execute('BEGIN IMMEDIATE')
    try:
        existing=a.db.execute('SELECT * FROM campaign_cost_revisions WHERE grant_id=?',(GRANT,)).fetchone()
        if existing:
        ...
        check_current(a,body)
        if not existing:a.db.execute('INSERT INTO campaign_cost_revisions VALUES(?,?,?,?,NULL,NULL)',
            (cid,GRANT,json.dumps(body,ensure_ascii=False,sort_keys=True),'claimed_not_dispatched'))
        a.db.execute('COMMIT')
...
def validate_proof(body,cid,jid,proof):
    ...
    group=groups[0]
    if (group.get('item')!=ITEM or group.get('offer_id')!=OFFER or group.get('state')!='verified_saved'
            or group.get('window',{}).get('offer_id')!=OFFER
            or any(group.get('window',{}).get(k)!=v for k,v in WINDOW.items())
            or set(group.get('values',{}))!=set(SKUS) or set(group.get('before',{}))!=set(SKUS)):
...
def record(a,cid,job):
    ...
    validate_proof(body,cid,row['job_id'],proof)
...
def overlay(a,offers):
    ...
    for row in a.db.execute("SELECT body FROM campaign_cost_revisions WHERE state='dispatched_unknown'"):
        ...
        for offer in offers:
            if offer.get('platform_offer_id',offer['offer_id'])==OFFER and all(offer[k]==body[k] for k in ('start','end')):
                for item in offer['items']:
                    if item['item']==ITEM:item['status']='unknown'
    ...
    for row in a.db.execute("SELECT * FROM campaign_cost_revisions WHERE state='verified'"):
        ...
        validate_proof(body,row['id'],row['job_id'],load(ref['path']))
        for change in body['rows']:
            candidates=[r for o in offers if o.get('platform_offer_id',o['offer_id'])==OFFER
                and (o['start'],o['end'])==(WINDOW['start'],WINDOW['end']) for r in o['rows']
                if (r['item'],r['sku'])==(ITEM,change['sku'])]
