...
$remote = 'sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_correct_plan7_small_promo'
...
try {
    ...
    if ($exitCode -ne 0) {
        throw "计划7小促价修正失败，退出码 $exitCode。该入口禁止自动重跑，请把上方完整回执交给 02。"
    }
} finally {
