...
def validated_body(authority, identity, phase, *, revalidate_reuse=True):
    ...
    if revalidate_reuse:
        ...
        # A consumed dispatch is verified against its immutable bundle and
        # exact generated bytes. Repartitioning it with NOW's expiring offer
        # observations is a new-submission check, not receipt verification.
        # This branch never claims, consumes, releases or submits anything.
        planned={(r['item'],r['sku']):r for r in discounts}
        if any(planned.get((r['item'],r['sku']))!=r for r in body['discount_rows']):
            raise ValueError('dispatched_discount_formula_changed')
...
def verify_claim(authority, claim_id, *, dispatched_job=None):
    ...
    body, file = (validated_body(authority, first['bundle_id'], first['phase']) if dispatched_job is None
                  else validated_body(authority, first['bundle_id'], first['phase'],revalidate_reuse=False))
