...
$payload = [ordered]@{
    ...
    confirmation = if ($ReadbackOnly) {
        'READBACK_ONLY_PLAN8_V4_NO_PLATFORM_WRITE'
    } else {
        'EXECUTE_ONCE_PLAN8_V4_6_ITEMS_78_SKUS_18_CUSTOM'
    }
    target_scope_sha256 = 'b239dc515b0f2442257e90fe30a1cda95e29f6ffd2ea123d6c53f6fd6a4feb1d'
}
$raw = $payload | ConvertTo-Json -Compress
$remote = 'sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_recover_plan8_final_v4'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Plan 8 V4 command failed with exit code $LASTEXITCODE. If a write claim exists, use only -ReadbackOnly; never run -ExecuteOnce again."
    }
} finally {
