...
$payload = [ordered]@{
    ...
    confirmation = 'RESUME_ONCE_PLAN8_V8_AFTER_V17_UNPERSISTED_READBACK_V18'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Plan 8 V8 claimed-preupload V18 stopped with exit code $LASTEXITCODE. Do not run this command again; inspect the returned JSON."
    }
} finally {
