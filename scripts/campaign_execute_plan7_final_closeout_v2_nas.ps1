...
# V1 is permanently retired. V2 additionally binds the incident recovery id
# and the deployed Web-Agent repair commit before the service may read/write.
$payload = [ordered]@{
...
$remote = 'sudo -n /var/packages/ContainerManager/target/usr/bin/docker exec -i panse-system-api-1 python -m app.cli.campaign_execute_plan7_final_closeout_v2'
...
try {
    ...
    if ($exitCode -ne 0) {
        ...
        throw "计划7最终收口 V2 CLI 失败，退出码 $exitCode$responseJson"
    }
} finally {
