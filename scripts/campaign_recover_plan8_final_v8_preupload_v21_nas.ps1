...
$payload = [ordered]@{
    ...
    confirmation = 'RESUME_ONCE_PLAN8_V8_AFTER_V20_CLAIM_STEP_FIX_V21'
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Plan 8 V8 claimed-preupload V21 stopped with exit code $LASTEXITCODE. Do not run this command again; inspect the returned JSON."
    }
} finally {
